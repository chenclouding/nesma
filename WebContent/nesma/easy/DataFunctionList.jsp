...
EIF:一组逻辑上相关联的、被持久保存的数据，由其他系统维护，但被当前待估算系统使用。'" : ""}><s:property value="type"/></td> 
			     ...
			     <td class = editimg>
			         <a href="/nesma/nesma/main/DataFunctionE!edit.action?dataFunction.id=<s:property value="id"/>">
			           <IMG src="/nesma/images/btnedit.gif" alt="点击修改" border="0" />
			         </a>
			     </td>
			     <td class = delimg>
			         <a href="/nesma/nesma/main/DataFunctionE!del.action?dataFunction.id=<s:property value="id"/>&functionModule.id=<s:property value="functionModule.id"/>" onclick="return confirmdel()">
    ...
    <form  action="DataFunctionE!add.action" name="dfForm" id="dfForm" method="post" onSubmit="return verifyInput();">
  	<table>
  		<tr>

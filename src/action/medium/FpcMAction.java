...
public class FpcMAction extends ActionSupport {
	...
	public String input() {
		...
		TransFunctionBusiness tfb = new TransFunctionBusiness();
		...
		for (FunctionModule fm : functionModules) {
			...
			fm.setTransFunctions(new TransFunctionBusiness().getTransFunctionByCountSessionFunctionModule(countSession, fm));
			double df = 0;
	...
	private double getFM(CountSession countSession2, FunctionModule fm) {
		double fpc = 0;
		fpc += new DataFunctionBusiness().getUfpc(countSession2, fm);
		fpc += new TransFunctionBusiness().getUfpc(countSession2, fm);
		return fpc;
	}
	...
	private double getFMFit(CountSession countSession2, FunctionModule fm) {
		double fpc = 0;
		fpc += new DataFunctionBusiness().getUfpcFit(countSession2, fm);
		fpc += new TransFunctionBusiness().getUfpc(countSession2, fm);
		return fpc;
	}
